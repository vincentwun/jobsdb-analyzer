document.addEventListener('DOMContentLoaded', () => {
  ...
  document.querySelectorAll('input[name="pagesMode"]').forEach(el => {
    el.addEventListener('change', (e) => {
      ...
      if (numPagesInput) {
        ...
        if (isCustom) {
          // focus and select so user can immediately type
          setTimeout(() => {
            numPagesInput!.focus();
            try { numPagesInput!.select(); } catch (err) { }
          }, 0);
        }
      }
    });
  ...
  // Initialize numPages enabled state based on current selection (if page loaded with custom selected)
  try {
    const sel = document.querySelector('input[name="pagesMode"]:checked') as HTMLInputElement | null;
    if (sel && numPagesInput) {
      const isCustom = sel.value === 'custom';
      numPagesInput.disabled = !isCustom;
      if (isCustom) {
        setTimeout(() => { try { numPagesInput!.focus(); numPagesInput!.select(); } catch (e) { } }, 0);
      }
    }
  } catch (e) { /* ignore */ }
